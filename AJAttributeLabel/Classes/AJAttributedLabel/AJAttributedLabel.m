...
NSString *const AJSelectedLinkBackgroundColorAttributeName = @"AJSelectedLinkBackgroundColorAttributeName";
NSString *const AJBackgroundColorAttributeName = @"AJBackgroundColorAttributeName";
NSString *const AJBackgroundColorInsetsAttributeName = @"AJBackgroundColorInsetsAttributeName";
NSString *const AJBackgroundColorCornerRadiusAttributeName = @"AJBackgroundColorCornerRadiusAttributeName";

static NSString *const kAJAttachmentLinkUrl = @"AJAttachment://";

static NSString *const kAJAttributedLabelRangeKey = @"kAJAttributedLabelRangeKey";

static NSString *const AJLinkAttributeName = @"AJLinkAttributeName";

static CGFloat const kDefaultBackgroundColorCornerRadius = 3;

@interface AJAttributedLabel () <AJLayoutManagerDelegate>
/**
 TextKit三件套
 */
@property (nonatomic, strong) AJLayoutManager *layoutManager;
...
@implementation AJAttributedLabel
...
- (void)configureTextkitStack {
    ...
    self.layoutManager = [[AJLayoutManager alloc] init];
...
- (void)setSelectedRange:(NSRange)range
{
    ...
    if (self.selectedRange.length && !NSEqualRanges(self.selectedRange, range)) {
        [self.textStorage removeAttribute:AJSelectedLinkBackgroundColorAttributeName range:self.selectedRange];
    }
    // Apply the new selection to the text
    if (range.length) {
        [self.textStorage addAttribute:AJSelectedLinkBackgroundColorAttributeName value:self.linkTextAttributes[AJSelectedLinkBackgroundColorAttributeName] ? : [UIColor lightGrayColor] range:range];
    }
...
- (NSDictionary<NSString *,id> *)layoutLinkTextAttributes
{
    ...
    [attributes removeObjectForKey:AJSelectedLinkBackgroundColorAttributeName];
    return attributes;
}
...
- (void)detectLinkAtCharRange:(NSRange)range
{
    ...
    [linkDetector enumerateMatchesInString:self.textStorage.string options:0 range:range usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        if (result.URL) {
            ...
            [self.textStorage addAttribute:AJLinkAttributeName value:result.URL range:result.range];
        }
        if (result.phoneNumber) {
            [self.textStorage addAttributes:[self layoutLinkTextAttributes] range:result.range];
            [self.textStorage addAttribute:AJLinkAttributeName value:[NSURL URLWithString:[@"tel://" stringByAppendingString:result.phoneNumber]] range:result.range];
        }
    }];
}
...
- (void)updateLinkAttributesAtCharRange:(NSRange)charRange
{
    ...
    [self.textStorage enumerateAttribute:NSLinkAttributeName inRange:charRange options:NSAttributedStringEnumerationReverse usingBlock:^(id  _Nullable value, NSRange range, BOOL * _Nonnull stop) {
        if (value) {
            ...
            [self.textStorage addAttribute:AJLinkAttributeName value:value range:range];
        }
    }];
}
           ...
           withEvent:(UIEvent *)event
{
    ...
    if (!self.activeLinkAttributes) {
        ...
        self.selectedRange = [self.activeLinkAttributes[kAJAttributedLabelRangeKey] rangeValue];
    }
}
        ...
        if (self.activeLinkAttributes) {
            if (self.activeLinkAttributes[NSAttachmentAttributeName]) {
                ...
                if ([self.delegate respondsToSelector:@selector(attributedLabel:didTapLinkWithURL:)]) {
                    [self.delegate attributedLabel:self didTapLinkWithURL:self.activeLinkAttributes[AJLinkAttributeName]];
                }
            }
            self.activeLinkAttributes = nil;
...
{
    NSRange range;
    NSURL *linkURL = [layoutManager.textStorage attribute:AJLinkAttributeName atIndex:charIndex effectiveRange:&range];
...
- (NSDictionary *)linkAtPoint:(CGPoint)location
{
    ...
    if (characterIndex < self.textStorage.length) {
        NSRange range;
        id value = [self.textStorage attribute:AJLinkAttributeName atIndex:characterIndex longestEffectiveRange:&range inRange:NSMakeRange(0, self.textStorage.length)];
        if (!value) {
            return nil;
        }
        return @{AJLinkAttributeName: value,
                 kAJAttributedLabelRangeKey: [NSValue valueWithRange:range]
                 };
    }
    return nil;
}
...
@implementation AJLayoutManager

@dynamic delegate;

- (void)setDelegate:(id<AJLayoutManagerDelegate>)delegate
{
    [super setDelegate:delegate];
}

- (id<AJLayoutManagerDelegate>)delegate
{
    return (id<AJLayoutManagerDelegate>)[super delegate];
}
...
- (void)drawGlyphsForGlyphRange:(NSRange)glyphsToShow atPoint:(CGPoint)origin
{
    ...
    if ([self.delegate respondsToSelector:@selector(layoutManager:drawCustomViewAttachment:inRect:)]) {
        ...
        [self.textStorage enumerateAttribute:NSAttachmentAttributeName inRange:characterRange options:NSAttributedStringEnumerationReverse usingBlock:^(id  _Nullable value, NSRange range, BOOL * _Nonnull stop) {
            if (value && [value isKindOfClass:[AJTextAttachment class]] && range.location != NSNotFound && [(AJTextAttachment *)value customView]) {
...
- (void)drawBackgroundForGlyphRange:(NSRange)glyphsToShow atPoint:(CGPoint)origin
{
    ...
    NSRange characterRange = [self characterRangeForGlyphRange:glyphsToShow actualGlyphRange:nil];
    //先画背景，再画选中的背景
    NSArray *attributes = @[AJBackgroundColorAttributeName, AJSelectedLinkBackgroundColorAttributeName];
...
- (UIEdgeInsets)backgroundColorInsetsForRange:(NSRange)range
{
    ...
    id rectInset = [self.textStorage attribute:AJBackgroundColorInsetsAttributeName atIndex:range.location longestEffectiveRange:nil inRange:range];
...
- (CGFloat)backgroundColorCornerRadiusForRange:(NSRange)range
{
    ...
    id value = [self.textStorage attribute:AJBackgroundColorCornerRadiusAttributeName atIndex:range.location longestEffectiveRange:nil inRange:range];
...
@implementation AJTextAttachment

+ (AJTextAttachment *)attachmentWithCustomView:(UIView *)customView viewSize:(CGSize)viewSize
{
    AJTextAttachment *attachment = [[AJTextAttachment alloc] initWithData:nil ofType:nil];
